...
export const bindLoginForm = () => {
  ...
  const getFriendlyError = (action, error) => {
    const raw = String(error?.message || '').toLowerCase();
    if (action === 'login') {
      if (raw.includes('invalid login credentials')) {
        return 'بيانات الدخول غير صحيحة. إذا لم تنشئ الحساب بعد اضغط \"إنشاء حساب جديد\" أولاً.';
      }
      if (raw.includes('email not confirmed')) {
        return 'الحساب غير مفعل بعد. يرجى تأكيد البريد الإلكتروني أولًا.';
      }
      return mapSupabaseError(error, 'فشل تسجيل الدخول. تحقق من البيانات.');
    }

    if (raw.includes('user already registered')) {
      return 'هذا البريد مسجل مسبقًا. استخدم زر تسجيل الدخول مباشرة.';
    }
    if (raw.includes('signup is disabled') || raw.includes('signups not allowed')) {
      return 'إنشاء الحسابات معطّل في Supabase. فعّل Email Signup من إعدادات Authentication.';
    }
    return mapSupabaseError(error, 'فشل إنشاء الحساب. تحقق من إعدادات Supabase.');
  };

  const executeAuth = async (action) => {
    ...
    try {
      ...
      if (action === 'signup') {
        ...
        showToast('تم إنشاء الحساب. افحص البريد لتأكيد الحساب ثم اضغط تسجيل الدخول.');
        return;
      }
      ...
      showToast(getFriendlyError(action, error), true);
    }
  };

  form.addEventListener('submit', async (event) => {
    event.preventDefault();
    await executeAuth('login');
  });
